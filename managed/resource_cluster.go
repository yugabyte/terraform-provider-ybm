...
func resourceClusterRead(ctx context.Context, accountId string, projectId string, clusterId string, backUpSchedules []BackupScheduleInfo, regions []string, allowListProvided bool, inputAllowListIDs []string, readOnly bool, apiClient *openapiclient.APIClient) (cluster Cluster, readOK bool, errorMessage string) {
	...
	if cmkResp.Data.Spec.Get() != nil {
		cmkSpec := CMKSpec{}
		cmkRespSpec := cmkResp.Data.GetSpec()

		cmkSpec.ProviderType = types.String{Value: string(cmkRespSpec.GetProviderType())}
		cmkSpec.IsEnabled = types.Bool{Value: bool(cmkRespSpec.GetIsEnabled())}

		switch cmkSpec.ProviderType.Value {
		case "AWS":
			awsCMKSpec := AWSCMKSpec{
				AccessKey: types.String{Value: cmkRespSpec.GetAwsCmkSpec().AccessKey},
				SecretKey: types.String{Value: cmkRespSpec.GetAwsCmkSpec().SecretKey},
				ARNList:   []types.String{},
			}
			cmkSpec.AWSCMKSpec = &awsCMKSpec

			for _, arn := range cmkRespSpec.GetAwsCmkSpec().ArnList {
		...
		case "GCP":
			gcpCMKSpec := GCPCMKSpec{
				KeyRingName:     types.String{Value: cmkRespSpec.GetGcpCmkSpec().KeyRingName},
				KeyName:         types.String{Value: cmkRespSpec.GetGcpCmkSpec().KeyName},
				Location:        types.String{Value: cmkRespSpec.GetGcpCmkSpec().Location},
				ProtectionLevel: types.String{Value: cmkRespSpec.GetGcpCmkSpec().ProtectionLevel},
			}
			if cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount != nil {
				gcpServiceAccount := GCPServiceAccount{
					Type:                    types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.Type},
					ProjectId:               types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.ProjectId},
					PrivateKeyId:            types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.PrivateKeyId},
					ClientEmail:             types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.ClientEmail},
					ClientId:                types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.ClientId},
					AuthUri:                 types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.AuthUri},
					TokenUri:                types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.TokenUri},
					AuthProviderX509CertUrl: types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.AuthProviderX509CertUrl},
					ClientX509CertUrl:       types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.ClientX509CertUrl},
				}
				if cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.GetPrivateKey() != "" {
					privateKey := types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.GetPrivateKey()}
					gcpServiceAccount.PrivateKey = privateKey
				}
				if cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.GetUniverseDomain() != "" {
					universeDomain := types.String{Value: cmkRespSpec.GetGcpCmkSpec().GcpServiceAccount.GetUniverseDomain()}
