...
- `ignore_num_nodes_changes` (Boolean) When true, Terraform ignores num_nodes changes made outside Terraform, such as changes made by an ACTIVE autoscaler policy. Explicit num_nodes changes in the Terraform configuration are still applied. Set this to true for regions managed by an ACTIVE autoscaler policy.
- `is_default` (Boolean)
