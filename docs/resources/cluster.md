...
To prevent Terraform drift when a region uses an ACTIVE autoscaler policy
...
- `ignore_num_nodes_changes` (Boolean) When true, runtime changes to num_nodes made by the autoscaler are not treated as Terraform drift. Recommended for regions with an ACTIVE ybm_autoscaler_policy. The provider preserves the Terraform-managed num_nodes value in state on refresh. When num_nodes is not explicitly changed in configuration, edit cluster requests use the actual node count from YugabyteDB Aeon.
- `is_default` (Boolean)
